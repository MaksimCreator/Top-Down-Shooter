public static class Constant
{
    public const string MyBest = nameof(MyBest);

    public const float GainEnterAccaleration = 1.5f;
    public const float ZoneEnterSlowDown = 0.4f;
    public const float ZoneExitSlowDown = 2.5f;
    public const int WallInvisibleLayer = 3;
    public const int EnemyLayer = 6;
    public const int BulletLayer = 7;
    public const int PlayerLayer = 8;
    public const int WeaponLayer = 9;
    public const int StartSceneIndex = 0;
}